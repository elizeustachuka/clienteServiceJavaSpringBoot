...
import br.com.elizeustachuka.cliente.web.ApiError;
import br.com.elizeustachuka.cliente.web.ResponseEntityBuilder;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    // GatewayUnavailableException : triggers when the External Gateway is unavailable
    @ExceptionHandler(GatewayUnavailable.class)
    public ResponseEntity<Object> handleGatewayUnavailable(GatewayUnavailable ex) {

        List<String> details = new ArrayList<String>();
        details.add(ex.getMessage());

        ApiError err = new ApiError(LocalDateTime.now(),HttpStatus.GATEWAY_TIMEOUT, "Gateway is out" ,details);

        return ResponseEntityBuilder.build(err);
    }

    // ResourceIncorrectFormatException : triggers when the value CEP is Incorrect
    @ExceptionHandler(ResourceIncorrectFormatException.class)
    public ResponseEntity<Object> handleResourceIncorrectFormat(ResourceIncorrectFormatException ex) {

        List<String> details = new ArrayList<String>();
        details.add(ex.getMessage());

        ApiError err = new ApiError(LocalDateTime.now(),HttpStatus.BAD_REQUEST, "Value with incorrect format" ,details);
